<?php
/**
 * Feature Name:    Comment Functions for Theme
 * Version:		    0.1
 ...
 * @return void
 */
function cora_the_comment( $comment, Array $args, $depth ) {
	...
	<li <?php comment_class(); ?>>
		<article id="comment-<?php comment_ID(); ?>" class="clearfix">
			...
			<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="comment-awaiting-moderation alert-info"><?php _e( 'Your comment is awaiting moderation.', 'cora-2014' ); ?></p>
			<?php endif; ?>
			...
			<section class="comment-content">
				...
				comment_reply_link(
					array_merge(
						$args,
						array(
						     'reply_text'=> __( 'Auf diesen Kommentar antworten &raquo;', 'cora-2014' ),
						     'depth' 	=> $depth,
						     'max_depth'	=> 999
						)
					)
 ...
 * @return  int
 */
function cora_get_count_pings( $post_id = NULL ) {
	global $wp_query;
 ...
 * @return  Void
 */
function cora_the_pings( $comment ) {
	$url	    = esc_url( $comment->comment_author_url );
	$icon_args  = array( 'url' => $url );
	$icon	    = cora_get_external_favicon( $icon_args );
 ...
 * @return  string
 */
function cora_get_external_favicon( Array $args = array()  ) {
	...
	$rtn = apply_filters( 'pre_cora_get_external_favicon', FALSE, $args, $default_args );
	if ( $rtn !== FALSE )
		return $rtn;

	$args = wp_parse_args( $args, $default_args );
	$args = apply_filters( 'cora_get_external_favicon_args', $args );
	...
	return apply_filters( 'cora_get_external_favicon', $output, $args );
